...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{images/cnn.jpg}
...
\begin{figure}[H]
  ...
  \caption{โครงสร้างของ U-Net \cite{ronneberger2015unet}}
...
ใน LinkNet ใช้การ "บวก" (Addition) ข้อมูลจาก encoder เข้ากับ decoder แทนการ "ต่อกัน" (Concatenation) แบบ U-Net ซึ่งช่วยลดภาระหน่วยความจำ (Memory efficient) และลดปัญหา gradient vanishing นอกจากนี้ LinkNet ยังมีจำนวนพารามิเตอร์น้อย ทำให้เหมาะสำหรับการใช้งานที่ต้องการความเร็วสูงหรือมีข้อจำกัดด้านทรัพยากร
...
\begin{figure}[H]
  ...
  \caption{โครงสร้างของ LinkNet \cite{chaurasia2017linknet}}
...
\begin{figure}[H]
  ...
  \caption{โครงสร้างของ DeepLabV3+ \cite{chen2018deeplab}}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{images/upernet.png}
  \caption{โครงสร้างของ UPerNet \cite{xiao2018upernet}}
...
\subsection{การวิเคราะห์เชิงเปรียบเทียบของ Encoder / Backbone}
เพื่อให้เห็นภาพรวมในการเลือกใช้ Backbone สำหรับงานแบ่งส่วนภาพทางการแพทย์ โครงงานนี้จึงทำการสรุปข้อดีและข้อจำกัดของแต่ละโครงข่ายตามมิติต่าง ๆ ดังนี้
\begin{table}[hbt!]
\centering
\caption{การเปรียบเทียบคุณลักษณะของ Backbone แต่ละประเภท}
\label{tab:backbone_comparison}
\resizebox{\textwidth}{!}{%
\begin{tabular}{|l|c|c|c|l|}
\hline
\textbf{Backbone} & \textbf{ความซับซ้อน} & \textbf{ความแม่นยำ} & \textbf{ความเร็ว (Inference)} & \textbf{จุดเด่นที่สำคัญ} \\ \hline
ResNet50/101 & ปานกลาง-สูง & สูงมาก & ปานกลาง & มีความเสถียรสูง (Robust) และเป็นมาตรฐานสากล \\ \hline
MobileNetV2 & ต่ำ (Lightweight) & ปานกลาง & สูงมาก (Real-time) & ใช้ Parameter น้อยมาก เหมาะกับอุปกรณ์สเปกต่ำ \\ \hline
EfficientNet-B3 & ปานกลาง & สูงมาก & ปานกลาง-สูง & มีความสมดุลสูงสุดระหว่างขนาดและประสิทธิภาพ \\ \hline
EfficientNet-B7 & สูงมาก & สูงที่สุด & ต่ำ & ให้รายละเอียดประณีตที่สุดแต่ต้องการ GPU ขนาดใหญ่ \\ \hline
\end{tabular}
}
\end{table}

จากตารางที่ \ref{tab:backbone_comparison} จะเห็นได้ว่าการเลือกใช้ Backbone มีความสัมพันธ์แบบ Trade-off ดังนี้

\begin{itemize}
\item \textbf{ด้านประสิทธิภาพ (Performance):} หากงานวิจัยให้ความสำคัญกับค่า Dice Score หรือความละเอียดของขอบเขตอวัยวะเป็นหลัก EfficientNet-B7 และ ResNet101 จะเป็นตัวเลือกที่เหมาะสม เนื่องจากมีความลึกของชั้นกรองสัญญาณที่สามารถเรียนรู้ Pattern ที่ซับซ้อนได้ดีกว่า
\item \textbf{ด้านทรัพยากร (Resource Efficiency):} ในกรณีที่ต้องการนำโมเดลไปใช้ในระบบตรวจวินิจฉัยที่ต้องการผลลัพธ์ทันที (Real-time Diagnostic) MobileNetV2 จะมีความได้เปรียบเนื่องจากมีโครงสร้างแบบ Inverted Residual ที่ลดการคำนวณเชิงซ้อน
\item \textbf{ด้านความสมดุล (Scalability):} EfficientNet-B3 และ ResNet50 ถือเป็นจุดสมดุล (Sweet Spot) ที่ให้ความแม่นยำเพียงพอต่อการวินิจฉัยทางการแพทย์โดยที่ยังคงความเร็วในการฝึกสอน (Training time) ที่ไม่สูงจนเกินไป
\end{itemize}



\section{ฟังก์ชันการสูญเสีย (Loss Function)}
...
\section{การเชื่อมต่อของพิกเซล (Connectivity)}
...
ขณะเดียวกันในขั้นตอน post-processing Connectivity มีบทบาทสำคัญในการปรับปรุงผลลัพธ์ของการแบ่งส่วนภาพ โดยสามารถใช้ในการลบวัตถุขนาดเล็กที่ไม่เกี่ยวข้อง (small connected components removal) การรวมบริเวณที่ควรเป็นวัตถุเดียวกัน และการเติมช่องว่างภายในวัตถุ (hole filling) เพื่อให้ segmentation mask มีความสมบูรณ์และสอดคล้องกับลักษณะจริงของวัตถุมากยิ่งขึ้น



\section{ตัวชี้วัดการประเมิน (Evaluation Metrics)}
...
\subsection{เวลาในการประมวลผล (Inference Time)}
Inference Time หมายถึงระยะเวลาที่แบบจำลองใช้ในการประมวลผลข้อมูลอินพุตหนึ่งตัวอย่างเพื่อสร้างผลลัพธ์ (prediction) หลังจากที่ผ่านกระบวนการฝึกสอน (training) เรียบร้อยแล้ว โดยเป็นตัวชี้วัดสำคัญในการประเมินประสิทธิภาพของระบบในด้านความเร็วและความสามารถในการนำไปใช้งานจริง (deployment) \cite{goodfellow2016deep}

โดยทั่วไป Inference Time จะขึ้นอยู่กับหลายปัจจัย เช่น ความซับซ้อนของสถาปัตยกรรมของโมเดล จำนวนพารามิเตอร์ ขนาดของอินพุต และทรัพยากรฮาร์ดแวร์ที่ใช้ในการประมวลผล เช่น หน่วยประมวลผลกราฟิก (GPU) \cite{han2016deep} โมเดลที่มีความลึกหรือมีจำนวนพารามิเตอร์มาก เช่นโครงข่ายที่ใช้ backbone ขนาดใหญ่ มักให้ความแม่นยำสูง แต่แลกมาด้วยเวลาในการประมวลผลที่เพิ่มขึ้น

ในงานด้านการแบ่งส่วนภาพ (image segmentation) Inference Time มีความสำคัญอย่างยิ่ง โดยเฉพาะในงานที่ต้องการการประมวลผลแบบใกล้เวลาจริง (real-time) เช่น ระบบช่วยวินิจฉัยทางการแพทย์ หรือระบบที่ต้องตอบสนองอย่างรวดเร็วต่อผู้ใช้งาน \cite{litjens2017survey} ดังนั้นจึงมักมีการออกแบบหรือเลือกใช้โมเดลที่มีความสมดุลระหว่างความแม่นยำและความเร็ว เช่น การใช้ lightweight backbone อย่าง MobileNet แทนโมเดลขนาดใหญ่


\subsection{Hausdorff Distance}
Hausdorff Distance เป็นตัวชี้วัดที่ใช้ในการวัดความแตกต่างระหว่างสองเซตของจุด โดยมักถูกนำมาใช้ในงานประมวลผลภาพและการแบ่งส่วนภาพ (image segmentation) เพื่อประเมินความคลาดเคลื่อนของขอบเขตระหว่างผลลัพธ์ที่โมเดลทำนาย (prediction) และข้อมูลจริง (ground truth) \cite{huttenlocher1993comparing}

แนวคิดหลักของ Hausdorff Distance คือการวัด “ระยะทางที่มากที่สุด” ระหว่างจุดในเซตหนึ่งไปยังจุดที่ใกล้ที่สุดในอีกเซตหนึ่ง กล่าวคือ ใช้ในการวัดกรณีที่เลวร้ายที่สุด (worst-case error) ระหว่างสองรูปทรง ซึ่งเหมาะสำหรับการประเมินความแม่นยำของขอบเขตในงาน segmentation

ให้ $A$ และ $B$ เป็นเซตของจุดสองชุด เช่น เซตของพิกเซลบนขอบเขตของ prediction และ ground truth โดย Hausdorff Distance นิยามได้ดังนี้
\begin{equation}
H(A, B) = \max\left\{\sup_{a \in A}\inf_{b \in B} d(a, b), \sup_{b \in B} \inf_{a \in A} d(b, a)\right\}
\end{equation}
โดยที่ $d(a, b)$ คือระยะทางระหว่างจุด $a$ และ $b$ ซึ่งโดยทั่วไปนิยมใช้ระยะทางแบบ Euclidean และ $\sup$ และ $\inf$ หมายถึงค่าสูงสุดเชิงขอบเขต (supremum) และค่าต่ำสุดเชิงขอบเขต (infimum) ตามลำดับ

Hausdorff Distance สามารถตีความได้ว่าเป็นระยะทางที่มากที่สุดที่ต้อง “เลื่อน” เซตหนึ่งเพื่อให้ครอบคลุมอีกเซตหนึ่งได้อย่างสมบูรณ์ ซึ่งทำให้สามารถสะท้อนความคลาดเคลื่อนของขอบเขตได้อย่างชัดเจน โดยเฉพาะในกรณีที่มีจุดผิดพลาดเพียงบางส่วนแต่มีระยะห่างมาก
อย่างไรก็ตาม Hausdorff Distance มีความไวต่อ outliers กล่าวคือ หากมีพิกเซลเพียงจุดเดียวที่อยู่ห่างจากขอบเขตจริงมาก อาจทำให้ค่าที่ได้สูงผิดปกติ ดังนั้น ในงานวิจัยบางส่วนจึงมีการใช้ Modified Hausdorff Distance หรือ Percentile Hausdorff Distance เช่น HD95 (95th percentile) เพื่อลดผลกระทบของ outliers \cite{dubuisson1994modified}
ในงาน segmentation ทางการแพทย์ Hausdorff Distance ถูกนำมาใช้ร่วมกับตัวชี้วัดอื่น เช่น IoU หรือ Dice coefficient เพื่อให้สามารถประเมินทั้งความถูกต้องเชิงพื้นที่และความแม่นยำของขอบเขตได้อย่างครอบคลุม \cite{taha2015metrics}
